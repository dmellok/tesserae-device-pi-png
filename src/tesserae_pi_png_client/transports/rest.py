...
from ..mqtt_loop import PaintFn, decode_png, parse_frame_payload
...
def _wake_cycle(
    ...
    if code == 304:
        ...
        try:
            # Reuse the MQTT payload parser — same fields apply: rotate / scale /
            # bg / saturation control the local PNG transform pipeline.
            request = parse_frame_payload(json.dumps(body).encode("utf-8"))
            raw = client.download(
                request.url,
            ...
            transformed = apply_transforms(
                img,
                (status.panel_w, status.panel_h),
                rotate=request.rotate,
                scale=request.scale,
                bg=request.bg,
            )
            paint_fn(transformed, request.saturation)
        except (
